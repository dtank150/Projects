...
<head>
    ...
    {
        $cid = $_POST['cid'];
        $query1="DELETE FROM complaint WHERE c_id='$cid'";
        $result1=mysqli_query($conn,$query1);

        if($result1)
        {
            echo "<script>alert('Complain Deleted Successfully.');</script>";
        }
        else
        {
            echo "<script>alert('Error Deleted Complain.');</script>";
        }
    }
...
<body style="background-color: #dfdfdf">
    ...
    <div>
        <form style="float: right; margin-right: 100px; margin-top: 65px;" method="post">
            ...
            <input class="btn btn-danger btn-sm" type="submit" value="Delete" style="margin-top:2px; margin-left:20px;" name="delete">
        </form>
    </div>
